...
#    ifdef HID_OLED
typedef struct _Seriarl_hid_oled_t {
    Hid_oled_content_t content;
} Serial_hid_oled_t;

volatile Serial_hid_oled_t serial_hid_oled = {};
uint8_t volatile status_hid_oled           = 0;
#    endif
...
#    ifdef HID_OLED
    [PUT_HID_OLED] =
        {
            (uint8_t *)&status_hid_oled, sizeof(Serial_hid_oled_t), (uint8_t *)&serial_hid_oled, 0, NULL  // no slave to master transfer
        },
#    endif
};
...
// HID OLED communication

void transport_hid_oled_master(void) {
    if (hid_oled_change_flags() & HID_OLED_SLAVE_CHANGE) {
        hid_oled_get_slave_content((Hid_oled_content_t *)&serial_hid_oled.content);
        if (soft_serial_transaction(PUT_HID_OLED) == TRANSACTION_END) {
            hid_oled_reset_change_flags(HID_OLED_SLAVE_CHANGE);
        }
    }
}

void transport_hid_oled_slave(void) {
    if (status_hid_oled == TRANSACTION_ACCEPTED) {
        hid_oled_update_slave_content((Hid_oled_content_t *)&serial_hid_oled.content);
        status_hid_oled = TRANSACTION_END;
        hid_oled_set_change_flags(HID_OLED_SLAVE_CHANGE);
    }
}
#    else
#        define transport_hid_oled_master()
#        define transport_hid_oled_slave()
#    endif
